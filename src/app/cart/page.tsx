...
export default function CartPage() {
  ...
  const subtotal = currentCategoryItems.reduce(
    (acc, item) => acc + (item.product.discountPrice || item.product.price) * item.quantity,
    0
  );
  const shipping = currentCategoryItems.length > 0 ? 5.00 : 0;
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-8">
        ...
        <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-3">
            <div className="lg:col-span-2">
                {categories.map(cat => {
                    const categoryItems = items.filter(item => item.product.category === cat.value);
                    return (
